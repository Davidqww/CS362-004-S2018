...
const int CARDS_TO_DRAW = 1;
const int ADDITIONAL_ACTIONS = 2;
...
int checkVillageCard(int p, struct gameState *post) {
	...
	// Successfully running village cardEffect returns 0
	if (r != 0) {
		cardEffectFailCount++;
		testFailed = 1;
	}
	...
	pre.numActions = pre.numActions + ADDITIONAL_ACTIONS;
